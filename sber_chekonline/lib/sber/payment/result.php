...
if ($checkParams) {
    ...
    if ($arOrder['PAYED'] == "N") {
        ...
        //-------------Доработка под несколько оплат
        //
        $preOrder = explode ('/',$resultId);
        //print_r($preOrder);
        $resultId = $preOrder[0];
        $pay_id = $preOrder[1];
        //
        ...
        if (($response['errorCode'] == 0) && $orderTrue && (($response['orderStatus'] == 1) || ($response['orderStatus'] == 2))) {
            ...
            $order = \Bitrix\Sale\Order::load($resultId);
            $pay_collection = $order->getPaymentCollection();
            //print_r($pay_collection);
            $payment = $order->getPaymentCollection()->getItemById($pay_id);
            ...
            /*************************************ADD CASHBOX (Печать чека) ********************************************/
            /*---*/
            /*---*/
            global $APPLICATION, $USER;

            
            if (count($pay_collection) == 1) {
                $typeId = 'sell';//prepayment, 
            } else {
                $typeId = 'prepayment';
            }
            ...
            //echo '----'.$typeId;
                ...
                $orderId =  $resultId;
                $paymentData = false;//['PAYMENTS'];
                $shipmentData = array ($ship_id => array ('ID' => $ship_id)); //name="SHIPMENTS[4110][ID]"
                $entityCode = 'P_'.$pay_id;
            ...
            //------------------DEVELOPER ANTON-----------------------
                ...
                $getArrOrder = CSaleOrder::GetByID($order_number);
                $arTransaction = array(
                    'USER_ID' => $getArrOrder['USER_ID'],
                    'AMOUNT' => $response["amount"] / 100,
                    'CURRENCY' => $response["currency"],
                    'DEBIT' => 'Y',
                    'DESCRIPTION' => 'Sberbank: ' .  $response["paymentAmountInfo"]["paymentState"],
